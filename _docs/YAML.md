...
The YAML class supports arrays, integers, floats, booleans, strings, multi-line strings (identified using pipes; `|`), and hexadecimal data (identified via unquoted strings containing the hexadecimal data, beginning with `0x`). The YAML class does not support callables, closures, or objects.
